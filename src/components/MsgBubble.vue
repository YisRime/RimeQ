<template>
  ...
  <div v-if="isSystem" class="flex-center my-4 w-full select-none">
    <div class="bg-dim text-sub text-xs px-3 py-1 rounded-full shadow-sm">{{ rawText }}</div>
  </div>
  ...
  >
    <!-- 多选勾选框 -->
    <div v-if="selectionMode" class="flex-center px-2">
      <div
        class="w-5 h-5 rounded-full border-2 border-dim flex-center my-trans"
        :class="isSelected ? 'bg-primary border-primary' : 'bg-transparent'"
      >
        <div v-if="isSelected" class="i-ri-check-line text-white text-xs" />
      </div>
    </div>
    ...
    <div class="flex flex-col max-w-[75%] md:max-w-[60%] min-w-[60px]" :class="isMe ? 'items-end' : 'items-start'">
      <div v-if="!isMe" class="flex-x gap-2 mb-1 ml-1 select-none">
        <span class="text-xs text-sub font-medium">{{ msg.sender.card || msg.sender.nickname }}</span>
      ...
      >
        ...
        >
          <div class="w-10 h-10 bg-white/20 rounded flex-center text-xl"><div class="i-ri-file-line" /></div>
        ...
        <div v-else-if="[MsgType.Json, MsgType.Xml].includes(msgType)" class="flex gap-3 items-center min-w-[200px]">
        ...
        <div v-else class="text-sm text-dim italic px-2">[不支持的消息: {{ msgType }}]</div>
...
/**
 * 消息气泡组件
 * 支持 文本、图片、语音、文件、卡片、Markdown 等多种渲染模式
 */
...
const markdownContent = computed(
  () => parsedMsg.value.markdown || parsedMsg.value.raw.find((s: any) => s.type === 'markdown')?.data?.content || ''
)
...
const bubbleClass = computed(() => {
  ...
  if (isMe.value) return 'px-3 py-2 bg-primary text-white border-transparent rounded-tr-sm'
