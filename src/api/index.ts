...
export class ExtendedClient extends BaseClient {
  ...
  getGroupMsgHistory(group_id: number, message_seq?: number, count = 20) {
    return this.request<{ messages: any[] }>('get_group_msg_history', { group_id, message_seq, count })
  }

  /** 获取好友历史消息 (社区) */
  getFriendMsgHistory(user_id: number, message_seq?: number, count = 20) {
    return this.request<{ messages: any[] }>('get_friend_msg_history', { user_id, message_seq, count })
  }

  /** 标记消息已读 (Lagrange) */
  markMsgAsRead(message_id: number) {
    return this.request('mark_msg_as_read', { message_id })
  }

  /** 标记群消息已读 (NapCat) */
  markGroupMsgAsRead(group_id: number, message_id?: number) {
    return this.request('mark_group_msg_as_read', { group_id, message_id })
  }

  /** 标记私聊消息已读 (NapCat) */
  markPrivateMsgAsRead(user_id: number, message_id?: number) {
    return this.request('mark_private_msg_as_read', { user_id, message_id })
  }

  /** 标记所有消息已读 (NapCat) */
  markAllAsRead() {
    return this.request('_mark_all_as_read')
  }

  /** 设置表情回应 (Lagrange) */
  setGroupReaction(group_id: number, message_id: number, code: string, is_add = true) {
    return this.request('set_group_reaction', { group_id, message_id, code, is_add })
  }

  /** 设置消息表情回应 (NapCat) */
  setMsgEmojiLike(message_id: number, emoji_id: string, set = true) {
    return this.request('set_msg_emoji_like', { message_id, emoji_id, set })
  }

  /** 获取消息表情回应列表 (NapCat) */
  fetchEmojiLike(message_id: number, user_id: number, emojiType: number, count = 20) {
    return this.request<{ emojiLikesList: T.EmojiLikeUser[] }>('fetch_emoji_like', { message_id, user_id, emojiType, count })
  }

  /** 加入群表情接龙 (Lagrange) */
  joinGroupEmojiChain(group_id: number, message_id: number, emoji_id: number) {
    return this.request('.join_group_emoji_chain', { group_id, message_id, emoji_id })
  }

  /** 加入好友表情接龙 (Lagrange) */
  joinFriendEmojiChain(user_id: number, message_id: number, emoji_id: number) {
    return this.request('.join_friend_emoji_chain', { user_id, message_id, emoji_id })
  }

  /** 调用群机器人回调 (Lagrange) */
  sendGroupBotCallback(group_id: number, bot_id: number, data_1: string, data_2: string) {
    return this.request('send_group_bot_callback', { group_id, bot_id, data_1, data_2 })
  }

  /** 转发好友单条消息 (NapCat) */
  forwardFriendSingleMsg(user_id: number, message_id: number) {
    return this.request('forward_friend_single_msg', { user_id, message_id })
  }

  /** 转发群单条消息 (NapCat) */
  forwardGroupSingleMsg(group_id: number, message_id: number) {
    return this.request('forward_group_single_msg', { group_id, message_id })
  }
  ...
  deleteFriend(user_id: number) {
    return this.request('delete_friend', { user_id })
  }

  /** 删除单向好友 (Go-CQ) */
  deleteUnidirectionalFriend(user_id: number) {
    return this.request('delete_unidirectional_friend', { user_id })
  }

  /** 设置好友备注 (NapCat) */
  setFriendRemark(user_id: number, remark: string) {
    return this.request('set_friend_remark', { user_id, remark })
  }
  ...
  setDoubtFriendsAddRequest(request_id: string, approve = true) {
    return this.request('set_doubt_friends_add_request', { request_id, approve })
  }

  /** 好友戳一戳 (NapCat) */
  friendPoke(user_id: number) {
    return this.request('friend_poke', { user_id })
  }
  ...
  setGroupPortrait(group_id: number, file: string) {
    return this.request('set_group_portrait', { group_id, file })
  }

  /** 设置精华消息 (Go-CQ/NapCat) */
  setEssenceMsg(message_id: number) {
    return this.request('set_essence_msg', { message_id })
  }

  /** 移出精华消息 (Go-CQ/NapCat) */
  deleteEssenceMsg(message_id: number) {
    return this.request('delete_essence_msg', { message_id })
  }

  /** 获取精华消息列表 (Go-CQ/NapCat) */
  getEssenceMsgList(group_id: number) {
    return this.request<T.EssenceMsg[]>('get_essence_msg_list', { group_id })
  }

  /** 群打卡 (Go-CQ) */
  sendGroupSign(group_id: number) {
    return this.request('send_group_sign', { group_id })
  }

  /** 获取群公告 */
  async getGroupNotice(group_id: number) {
    try {
      const res = await this.request<T.GroupNotice[]>('get_group_notice', { group_id })
      return Array.isArray(res) ? res : []
    } catch {
      const res = await this.request<T.GroupNotice[]>('_get_group_notice', { group_id })
      return Array.isArray(res) ? res : []
    }
  }
  ...
  sendGroupNotice(group_id: number, content: string, image?: string) {
    return this.request('_send_group_notice', { group_id, content, image })
  }

  /** 删除群公告 (Go-CQ) */
  delGroupNotice(group_id: number, notice_id: string) {
    return this.request('_del_group_notice', { group_id, notice_id })
  }

  /** 获取群系统消息 (Go-CQ/NapCat) */
  getGroupSystemMsg() {
    return this.request<{ invited_requests: any[], join_requests: any[] }>('get_group_system_msg')
  }
  ...
  setGroupBotStatus(group_id: number, bot_id: number, enable: number) {
    return this.request('set_group_bot_status', { group_id, bot_id, enable })
  }

  /** 群组踢多人 (NapCat) */
  setGroupKickMembers(group_id: number, user_ids: number[], reject_add_request = false) {
    return this.request('set_group_kick_members', { group_id, user_ids, reject_add_request })
  }

  /** 设置机器人进群选项 (NapCat) */
  setGroupRobotAddOption(group_id: number, robot_member_switch = 0, robot_member_examine = 0) {
    return this.request('set_group_robot_add_option', { group_id, robot_member_switch, robot_member_examine })
  }

  /** 设置群添加选项 (NapCat) */
  setGroupAddOption(group_id: number, add_type: number, group_question?: string, group_answer?: string) {
    return this.request('set_group_add_option', { group_id, add_type, group_question, group_answer })
  }

  /** 设置群搜索 (NapCat) */
  setGroupSearch(group_id: number, no_code_finger_open = 0, no_code_finger_close = 0) {
    return this.request('set_group_search', { group_id, no_code_finger_open, no_code_finger_close })
  }

  /** 设置群备注 (NapCat) */
  setGroupRemark(group_id: number, remark: string) {
    return this.request('set_group_remark', { group_id, remark })
  }

  /** 群内戳一戳 (NapCat) */
  groupPoke(group_id: number, user_id: number) {
    return this.request('group_poke', { group_id, target_id: user_id })
  }

  /** 获取群信息扩展 (NapCat) */
  getGroupInfoEx(group_id: number) {
    return this.request('get_group_info_ex', { group_id })
  }

  /** 获取群详细信息 (NapCat) */
  getGroupDetailInfo(group_id: number) {
    return this.request('get_group_detail_info', { group_id })
  }

  /** 获取被过滤的加群请求 (NapCat) */
  getGroupIgnoreAddRequest() {
    return this.request<any[]>('get_group_ignore_add_request')
  }
  ...
  getGroupIgnoredNotifies(group_id: number) {
    return this.request('get_group_ignored_notifies', { group_id })
  }
  ...
  getPrivateFileUrl(user_id: number, file_id: string) {
    return this.request<{ url: string }>('get_private_file_url', { user_id, file_id })
  }

  /** 上传群文件 */
  uploadGroupFile(group_id: number, file: string, name: string, folder?: string) {
    return this.request('upload_group_file', { group_id, file, name, folder })
  }
  ...
  getGroupRootFiles(group_id: number) {
    return this.request<T.GroupFiles>('get_group_root_files', { group_id })
  }

  /** 获取群子目录文件 */
  getGroupFilesByFolder(group_id: number, folder_id: string, file_count = 50) {
    return this.request<T.GroupFiles>('get_group_files_by_folder', { group_id, folder_id, file_count })
  }

  /** 删除群文件 */
  deleteGroupFile(group_id: number, file_id: string, busid?: number) {
    return this.request('delete_group_file', { group_id, file_id, busid })
  }

  /** 创建群文件夹 */
  createGroupFileFolder(group_id: number, name: string, parent_id = '/') {
    return this.request('create_group_file_folder', { group_id, name, parent_id })
  }

  /** 删除群文件夹 */
  deleteGroupFolder(group_id: number, folder_id: string) {
    return this.request('delete_group_folder', { group_id, folder_id })
  }

  /** 上传私聊文件 */
  uploadPrivateFile(user_id: number, file: string, name: string) {
    return this.request('upload_private_file', { user_id, file, name })
  }

  /** 上传图片 (Lagrange) */
  uploadImage(file: string) {
    return this.request('upload_image', { file })
  }

  /** 移动群文件 (社区) */
  moveGroupFile(group_id: number, file_id: string, folder_id: string) {
    return this.request('move_group_file', { group_id, file_id, folder_id })
  }

  /** 重命名群文件夹 (Lagrange) */
  renameGroupFileFolder(group_id: number, folder_id: string, new_name: string) {
    return this.request('rename_group_file_folder', { group_id, folder_id, new_name })
  }

  /** 转发群文件 (NapCat) */
  transGroupFile(group_id: number, file_id: string) {
    return this.request('trans_group_file', { group_id, file_id })
  }

  /** 重命名群文件 (NapCat) */
  renameGroupFile(group_id: number, file_id: string, new_name: string, parent_dir?: string) {
    return this.request('rename_group_file', { group_id, file_id, new_name, current_parent_directory: parent_dir })
  }

  /** 获取文件详情 (NapCat) */
  getFile(file_id: string) {
    return this.request('get_file', { file_id })
  }
  ...
  getOnlineClients(no_cache = false) {
    return this.request('get_online_clients', { no_cache })
  }

  /** 获取企点账号信息 (Go-CQ) */
  qidianGetAccountInfo() {
    return this.request('qidian_get_account_info')
  }

  /** 获取在线机型 (Go-CQ) */
  getModelShow(model: string) {
    return this.request('_get_model_show', { model })
  }

  /** 设置在线机型 (Go-CQ) */
  setModelShow(model: string, model_show: string) {
    return this.request('_set_model_show', { model, model_show })
  }

  /** 设置登录号资料 (Go-CQ/NapCat) */
  setQqProfile(profile: { nickname?: string, company?: string, email?: string, personal_note?: string, sex?: number }) {
    return this.request('set_qq_profile', profile)
  }

  /** 获取已收藏的QQ表情列表 (社区) */
  fetchCustomFace(count = 48) {
    return this.request('fetch_custom_face', { count })
  }

  /** 获取商城表情 key (Lagrange) */
  getMfaceKey(emoji_ids: string[]) {
    return this.request('get_mface_key', { emoji_ids })
  }

  /** 设置QQ头像 (社区) */
  setQqAvatar(file: string) {
    return this.request('set_qq_avatar', { file })
  }

  /** 获取rkey (社区) */
  getRkey() {
    return this.request('get_rkey')
  }

  /** 获取NC版rkey (NapCat) */
  ncGetRkey() {
    return this.request('nc_get_rkey')
  }

  /** 获取rkey服务器 (NapCat) */
  getRkeyServer() {
    return this.request('get_rkey_server')
  }

  /** 设置自定义在线状态 (NapCat) */
  setDiyOnlineStatus(wording: string, face_id?: number | string) {
    return this.request('set_diy_online_status', { wording, face_id })
  }

  /** 设置在线状态 (NapCat) */
  setOnlineStatus(status: number, ext_status = 0, battery_status = 0) {
    return this.request('set_online_status', { status, ext_status, battery_status })
  }

  /** 设置输入状态 (NapCat) */
  setInputStatus(event_type: number) {
    return this.request('set_input_status', { event_type })
  }

  /** 获取个人资料点赞 (NapCat) */
  getProfileLike(user_id?: number, start = 0, count = 10) {
    return this.request('get_profile_like', { user_id, start, count })
  }

  /** 获取官方机器人账号范围 (NapCat) */
  getRobotUinRange() {
    return this.request('get_robot_uin_range')
  }

  /** 设置自己的个性签名 (NapCat) */
  setSelfLongnick(longNick: string) {
    return this.request('set_self_longnick', { longNick })
  }

  /** 获取最近联系人 (NapCat) */
  getRecentContact(count = 20) {
    return this.request('get_recent_contact', { count })
  }

  /** 获取用户状态 (NapCat) */
  getUserStatus(user_id: number) {
    return this.request('nc_get_user_status', { user_id })
  }

  /** 获取 clientkey (NapCat) */
  getClientkey() {
    return this.request('get_clientkey')
  }
  ...
  downloadFile(url: string, thread_count = 1, headers?: string | string[]) {
    return this.request<{ file: string }>('download_file', { url, thread_count, headers })
  }
  ...
  handleQuickOperation(context: any, operation: any) {
    return this.request('.handle_quick_operation', { context, operation })
  }

  /** 获取中文分词 (Go-CQ) */
  getWordSlices(content: string) {
    return this.request<{ slices: string[] }>('.get_word_slices', { content })
  }
  ...
  clickInlineKeyboardButton(group_id: number, bot_appid: string, button_id: string, button_data: string) {
    return this.request('click_inline_keyboard_button', { group_id, bot_appid, button_id, button_data })
  }

  /** 获取推荐好友卡片 (NapCat) */
  arkSharePeer(user_id: number, phoneNumber?: string) {
    return this.request('ArkSharePeer', { user_id, phoneNumber })
  }

  /** 获取推荐群聊卡片 (NapCat) */
  arkShareGroup(group_id: number, phoneNumber?: string) {
    return this.request('ArkShareGroup', { group_id, phoneNumber })
  }

  /** 创建收藏 (NapCat) */
  createCollection(rawData: string, brief: string) {
    return this.request('create_collection', { rawData, brief })
  }
  ...
  /** 退出机器人 (NapCat) */
  botExit() {
    return this.request('bot_exit')
  }

  /** 发送自定义组包 (NapCat) */
  sendPacket(params: any) {
    return this.request('send_packet', params)
  }

  /** 获取 packet 状态 (NapCat) */
  ncGetPacketStatus() {
    return this.request('nc_get_packet_status')
  }

  /** 获取小程序卡片 (NapCat) */
  getMiniAppArk(params: any) {
    return this.request('get_mini_app_ark', params)
  }

  /** 获取 AI 语音 (NapCat) */
  getAiRecord(character: string, group_id: number, text: string) {
    return this.request<string>('get_ai_record', { character, group_id, text })
  }

  /** 发送戳一戳 (NapCat) */
  sendPoke(user_id: number, group_id?: number, target_id?: number) {
    return this.request('send_poke', { user_id, group_id, target_id })
  }

  /** 获取 AI 声色列表 (社区) */
  getAiCharacters(group_id?: number, chat_type = 1) {
    return this.request<T.AiCharacter[]>('get_ai_characters', { group_id, chat_type })
  }

  /** 发送群 AI 语音 (社区) */
  sendGroupAiRecord(group_id: number, character: string, text: string, chat_type = 1) {
    return this.request('send_group_ai_record', { group_id, character, text, chat_type })
  }
}
