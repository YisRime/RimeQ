...
const handleLogin = async (isAuto = false) => {
  if (!form.connectAddress) {
    if (!isAuto) toast.add({ severity: 'warn', summary: '请输入地址', life: 3000 })
    return
  }
  loading.value = true
  try {
    ...
    if (!isAuto) {
      toast.add({ severity: 'error', summary: '连接失败', detail: e || String(e), life: 3000 })
    }
    loading.value = false
  }
}
...
onMounted(() => {
  ...
  if (settingStore.config.rememberToken) {
    ...
    form.connectAddress = settingStore.config.connectAddress
  }
  // 触发自动登录
