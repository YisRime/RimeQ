<template>
  <div class="h-full w-full bg-sub flex flex-col">
    ...
    <header class="h-14 border-b border-dim bg-main/90 backdrop-blur flex-between px-4 flex-shrink-0 z-10">
      ...
      <div class="text-xs text-sub">共 {{ dataStore.notices.value.length }} 条</div>
    </header>

    <!-- List -->
    <div class="flex-1 overflow-y-auto my-scrollbar">
      <div class="p-4 flex flex-col gap-3">
        <div v-if="dataStore.notices.value.length === 0" class="text-center text-sub py-20">暂无新通知</div>

        <div
          v-for="(item, idx) in dataStore.notices.value"
          :key="idx"
...
const handleRequest = async (item: SystemNotice, idx: number, approve: boolean) => {
  ...
  try {
    ...
    dataStore.notices.value.splice(idx, 1)
    processing.value = {}
  } catch (e) {
    console.error(e)
